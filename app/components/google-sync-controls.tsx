...
import { pushNoteToGoogleDrive, getAllTags } from '@/(panel)/actions';
...
export const useGoogleSync = (noteId: string, note?: Note | null) => {
    ...
    const pushToGoogle = useCallback(async (options: {
        ...
        try {
            ...
            const result = await pushNoteToGoogleDrive(noteId, {
                ...
                tabName: options.tabName || (isGoogle ? `Update - ${new Date().toLocaleDateString()}` : undefined)
            });
    ...
    const assignGoogleDoc = useCallback(async (googleDocId: string) => {
        setIsPushing(true);
        setError(null);
        try {
