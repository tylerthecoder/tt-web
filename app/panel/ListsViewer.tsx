...
import { ListItem } from '../lists/[id]/list-item';
import { AddItemForm } from '../lists/[id]/add-item-form';
...
export function ListsViewer({ initialLists }: ListsViewerProps) {
                                    ...
                                    <div className="space-y-1">
                                        {list.items.slice(0, 3).map((item) => (
                                            <div key={item.id} className="flex items-center gap-2 text-xs text-gray-300 min-w-0">
                                                ...
                                                <span className={`truncate flex-1 min-w-0 ${item.checked ? 'line-through text-gray-500' : ''}`}>
                                                    {item.content}
                                                </span>
                                            </div>
                                        ))}
